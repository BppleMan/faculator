...
impl Commander for AtlasCommand {
    fn execute(&self) -> Result<()> {
        self.run(config::icons_dir(), config::icon_atlas_dir())
    }
}

impl AtlasCommand {
    fn run(&self, input: &Path, output: &Path) -> Result<()> {
        eprintln!("📂 扫描图标源目录: {}", input.display());
        let icons = self.scan_curated_icons(input)?;

        if icons.is_empty() {
            bail!("在 {} 中未找到任何 PNG 图标", input.display());
        }

        let grouped_icons = self.group_icons_by_size(icons);
        self.print_size_group_stats(&grouped_icons);
        let atlas_groups = self.build_atlas_groups(grouped_icons);
        self.print_atlas_group_stats(&atlas_groups);

        self.reset_output_dir(output)?;

        let manifest = self.build_grouped_manifest(&atlas_groups, output)?;
    ...
    fn scan_curated_icons(&self, input: &Path) -> Result<Vec<CuratedIcon>> {
        ...
        for entry in WalkDir::new(input).min_depth(2) {
            ...
            let key = self.sprite_key_from_relative_path(relative_path)?;
            let (width, height) =
    ...
    fn sprite_key_from_relative_path(&self, relative_path: &Path) -> Result<String> {
    ...
    fn group_icons_by_size(&self, icons: Vec<CuratedIcon>) -> BTreeMap<SizeKey, Vec<CuratedIcon>> {
    ...
    fn print_size_group_stats(&self, grouped_icons: &BTreeMap<SizeKey, Vec<CuratedIcon>>) {
        eprintln!(
            "   共 {} 张图标，归为 {} 个尺寸组",
    ...
    fn build_atlas_groups(&self, grouped_icons: BTreeMap<SizeKey, Vec<CuratedIcon>>) -> Vec<AtlasGroupInput> {
    ...
    fn print_atlas_group_stats(&self, atlas_groups: &[AtlasGroupInput]) {
        eprintln!("\n🗃️  图集分组:");
    ...
    fn reset_output_dir(&self, output: &Path) -> Result<()> {
        if output.exists() {
    ...
    fn build_grouped_manifest(&self, atlas_groups: &[AtlasGroupInput], output: &Path) -> Result<AtlasManifest> {
        ...
        for group in atlas_groups {
            ...
            let group_manifest = self.build_single_group_manifest(group, output)?;
    ...
    fn build_single_group_manifest(&self, group: &AtlasGroupInput, output: &Path) -> Result<AtlasGroupManifest> {
        let mut packer = MultiTexturePacker::new_skyline(self.texture_packer_config());
    ...
    for (page_index, page) in packer.get_pages().iter().enumerate() {
        ...
        self.render_page_image(page, &source_paths)?
            .save(&page_path)
    ...
    fn texture_packer_config(&self) -> TexturePackerConfig {
        TexturePackerConfig {
...
mod tests {
    ...
    fn run_atlas_groups_sprites_by_size() {
        ...
        command.run(&icons_dir, &atlas_output_dir).unwrap();
    ...
    fn sprite_key_uses_relative_path_without_extension() {
        ...
        let key = command
            .sprite_key_from_relative_path(std::path::Path::new("item/iron-plate.png"))
            .unwrap();
